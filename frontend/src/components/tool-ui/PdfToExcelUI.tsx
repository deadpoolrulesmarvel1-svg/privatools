...
import { Upload, Download, Loader2, AlertCircle, Table } from "lucide-react";
...
export function PdfToExcelUI() {
    ...
    const process = async () => {
        ...
        try { await processAndDownload("/pdf-to-excel", file, file.name.replace(/\.pdf$/i, ".xlsx")); setStatus("done"); }
        catch (e: any) { setError(e.message || "Failed"); setStatus("idle"); }
    };

    if (status === "done") return (
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-8 text-center">
            <Table size={28} className="mx-auto mb-3 text-emerald-400" />
            <h2 className="text-lg font-bold text-foreground mb-1">Converted!</h2>
            <p className="text-sm text-muted-foreground mb-4">Your Excel spreadsheet has been downloaded</p>
    ...
    return (
        <div className="space-y-4">
          ...
          aria-label="Upload file"
                ...
                <p className="text-[11px] text-muted-foreground/80">Extracts tables into .xlsx spreadsheet format</p>
            </div>
            ...
            {file && (
                <div className="flex items-center gap-3">
                    <Button onClick={process} disabled={status === "processing"} className="glow-primary">
                        {status === "processing" ? <><Loader2 size={15} className="animate-spin" /> Converting…</> : "Convert to Excel"}
                    </Button>
