  ...
  if (status === "done") {
    return (
      <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-8 text-center">
        <Download size={30} className="mx-auto mb-3 text-emerald-400" />
        <h2 className="mb-1 text-lg font-bold text-foreground">Pages Extracted</h2>
        <p className="mb-5 text-sm text-muted-foreground">Your extracted PDF has been downloaded.</p>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <label className="text-sm font-semibold text-foreground">Pages to extract</label>
        <input
          value={pages}
      ...
      <Button onClick={process} disabled={!file || status === "processing"} className="glow-primary">
        {status === "processing" ? <><Loader2 size={15} className="animate-spin" />Extracting…</> : "Extract Pages"}
      </Button>
    </div>
  );
}
