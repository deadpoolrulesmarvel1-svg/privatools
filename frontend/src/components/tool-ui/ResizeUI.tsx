...
export function ResizeUI() {
  ...
  if (state === "done") return (
    <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-10 text-center">
      ...
      <h2 className="text-lg font-bold text-foreground mb-1">Resized!</h2>
      <Button variant="outline" className="border-border text-muted-foreground mt-4" onClick={() => { setFile(null); setState("idle"); }}>Resize another</Button>
    </div>
  );
    ...
    <div className="space-y-4">
      {!file ? (
        <div onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)}
          ...
          <p className="text-sm font-semibold text-foreground">Select a PDF to resize</p><p className="text-xs text-muted-foreground">Drag & drop or click to browse</p>
        </div>
      ) : (
        <>
          ...
          <div className="rounded-xl border border-border bg-card p-5 space-y-4">
            <div><label className="text-sm font-semibold text-foreground">Page size</label>
                ...
                {sizes.map(s => (
                  <button key={s.id} onClick={() => setPageSize(s.id)}
                    className={cn("rounded-lg border px-4 py-2 text-xs font-medium transition-all",
                      pageSize === s.id ? "border-accent bg-accent/10 text-primary" : "border-border text-muted-foreground hover:bg-secondary/40")}>
                    {s.label}
                  </button>
                ))}
              </div>
          ...
          <Button onClick={process} disabled={state === "processing"} className="glow-primary">
            {state === "processing" ? <><Loader2 size={15} className="animate-spin" />Resizing…</> : "Resize PDF"}
          </Button>
        </>
      )}
    </div>
