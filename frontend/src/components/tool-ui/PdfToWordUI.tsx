...
export function PdfToWordUI() {
    ...
    const process = async () => {
        ...
        setError(null);
        try {
            await processAndDownload("/pdf-to-word", file, file.name.replace(/\.pdf$/i, ".docx"));
            setStatus("done");
    ...
    if (status === "done") return (
        <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-8 text-center">
            <FileText size={28} className="mx-auto mb-3 text-emerald-400" />
            <h2 className="text-lg font-bold text-foreground mb-1">Converted!</h2>
            <p className="text-sm text-muted-foreground mb-4">Your Word document has been downloaded</p>
        ...
        <div className="space-y-4">
          ...
          aria-label="Upload file"
            ...
            >
                ...
                <p className="text-[11px] text-muted-foreground/80">Converts to .docx Word format</p>
            </div>
            ...
            {file && (
                <div className="flex items-center gap-3">
                    <Button onClick={process} disabled={status === "processing"} className="glow-primary">
                        {status === "processing" ? <><Loader2 size={15} className="animate-spin" /> Converting…</> : "Convert to Word"}
                    </Button>
