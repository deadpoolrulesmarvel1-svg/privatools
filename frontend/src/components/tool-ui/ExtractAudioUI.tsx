                    ...
                    <div className="flex items-start gap-5">
                        ...
                        <div className="flex-1 min-w-0">
                            ...
                            <h2 className="font-display text-[26px] font-bold text-foreground tracking-[-0.025em] leading-tight" style={{ fontVariationSettings: '"opsz" 144, "SOFT" 50' }}>
                                {isMulti
                                    ? <><span className="italic text-accent">{proc.doneCount}</span> track{proc.doneCount === 1 ? "" : "s"} saved as <span className="italic text-accent">.{format}</span>{proc.failedCount > 0 ? <> · <span className="text-destructive italic">{proc.failedCount} failed</span></> : null}</>
    ...
    return (
        <div className="space-y-4">
            <FileUploadZone
                ...
                label={proc.entries.length ? "Add more files" : "Drop video to extract audio"}
                    ...
                    {objectUrl && previewFile && (
                        <div className="rounded-xl border border-border bg-card overflow-hidden">
                            ...
                            <div className="bg-black/95">
                                <video src={objectUrl} controls playsInline className="w-full max-h-[40vh]" aria-label={`Preview ${previewFile.name}`} />
                            </div>
                        </div>
                    )}
                    <div className="rounded-xl border border-border bg-card overflow-hidden">
                    ...
                    <div className="flex items-center gap-3 flex-wrap">
                        <button onClick={() => void process(false)} disabled={!canProcess} className="btn-accent disabled:opacity-60 disabled:cursor-not-allowed">
                            {phase === "processing"
                                ...
                                : <><Music size={13} /> Extract audio as {format.toUpperCase()}{proc.entries.length > 1 ? ` — ${proc.entries.length} files` : ""}</>}
                        </button>
                        {canProcess && (
