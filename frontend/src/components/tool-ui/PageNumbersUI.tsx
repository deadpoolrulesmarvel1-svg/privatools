...
const posOptions = [
  ...
  { id: "top-center", label: "Top Center" }, { id: "top-left", label: "Top Left" }, { id: "top-right", label: "Top Right" },
];
...
export function PageNumbersUI() {
  ...
  if (state === "done") return (
    <div className="rounded-2xl border border-emerald-500/20 bg-emerald-500/5 p-10 text-center">
      ...
      <h2 className="text-lg font-bold text-foreground mb-1">Page numbers added!</h2>
      <p className="text-sm text-muted-foreground mb-6">Your numbered PDF has been downloaded.</p>
      <Button variant="outline" className="border-border text-muted-foreground" onClick={() => { setFile(null); setState("idle"); }}>Number another</Button>
    </div>
  );
    ...
    <div className="space-y-4">
      {!file ? (
        <div onDragOver={e => { e.preventDefault(); setDrag(true); }} onDragLeave={() => setDrag(false)}
          ...
          <p className="text-sm font-semibold text-foreground">Select a PDF</p>
        ...
        <>
          ...
          <Button onClick={process} disabled={state === "processing"} className="glow-primary">
            {state === "processing" ? <><Loader2 size={15} className="animate-spin" />Adding…</> : "Add Page Numbers"}
          </Button>
        </>
      )}
    </div>
