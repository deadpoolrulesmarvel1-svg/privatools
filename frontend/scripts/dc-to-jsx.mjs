...
function convert(html) {
  ...
  while ((m = tok.exec(html))) {
    ...
    if (iconDepth < 0 && /material[\s-]symbols/i.test(attrs)) iconDepth = stack.length;
    stack.push(tag);
    out += `<${tag}${attrStr}>`;
  }
...
/* ── CLI ──────────────────────────────────────────────────────────────── */
const [design, startMarker, endMarker] = process.argv.slice(2);
if (!design || !FILES[design]) {
  console.error(`usage: node scripts/dc-to-jsx.mjs <${Object.keys(FILES).join("|")}> <start> [end]`);
  process.exit(1);
}
const src = readFileSync(FILES[design], "utf8");
const body = src.split("</style>")[1] ?? src;
const from = startMarker ? body.indexOf(startMarker) : 0;
if (from === -1) { console.error(`start marker not found: ${startMarker}`); process.exit(1); }
const to = endMarker ? body.indexOf(endMarker, from) : body.length;
if (to === -1) { console.error(`end marker not found: ${endMarker}`); process.exit(1); }
console.log(convert(body.slice(from, to)));
