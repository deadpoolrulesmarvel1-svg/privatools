...
TOOL_HOWTO: dict[str, list[dict[str, str]]] = {
    ...
    "unlock-pdf": [
        {"name": "Drop the locked PDF", "text": "Select the password-protected PDF file. The encrypted PDF enters temp memory only for the decryption pass."},
    ...
    "watermark": [
        {"name": "Drop your PDF", "text": "Select a PDF up to 500 MB that needs a watermark. The file enters temp memory only for the watermarking pass."},
    ...
    "jpg-to-pdf": [
        {"name": "Drop your JPG images", "text": "Upload one or many .jpg / .jpeg files (PNG also works) up to 500 MB total. Photos enter temp memory only for the conversion pass."},
    ...
    "pdf-to-jpg": [
        {"name": "Drop your PDF", "text": "Upload a PDF up to 500 MB. The file enters temp memory only for the conversion pass — no copy is kept after your download."},
...
TOOL_FAQ: dict[str, list[dict[str, str]]] = {
    "merge-pdf": [
        ...
        {"q": "Is it safe to upload sensitive PDFs to be merged?", "a": "Yes. Files enter an isolated Docker container, are processed in temporary memory, and are unlinked the moment the response is delivered. They are never written to permanent storage, never logged, and never used to train any model. The merger code is open source on GitHub."},
    ...
    "split-pdf": [
        ...
        {"q": "Is my PDF uploaded to the cloud?", "a": "It is uploaded to PrivaTools' own isolated container for processing, held in temp memory only, then unlinked immediately. It is never persisted to disk, never inspected, never shared with any third party. The split logic is open source on GitHub for verification."},
    ...
    "edit-pdf": [
        ...
        {"q": "Is it safe to edit a confidential PDF here?", "a": "Yes. The PDF is held in temp memory inside an isolated Docker container only for the duration of your edit session — both input and output are unlinked the moment your download begins. No copy is kept on disk, in logs, or in backups. The editor code is open source under MIT for verification."},
    ...
    "sign-pdf": [
        ...
        {"q": "Is my signature stored on the server?", "a": "No. Your signature exists only in your browser session — drawn on a canvas, kept in memory while you place it. The server receives the signature image only for the brief moment it embeds it into the PDF, then unlinks both input and output within seconds of your download. The signature image is never persisted to disk."},
        {"q": "Is it safe to sign a sensitive contract here?", "a": "Yes. The PDF and signature image are held in temp memory inside an isolated Docker container for the duration of the request only; both are unlinked the moment your download begins. Nothing is logged, indexed, or sent to a third-party API. The signing pipeline is open source on GitHub and you can self-host the entire stack."},
    ...
    "protect-pdf": [
        ...
        {"q": "Is it safe to upload a sensitive PDF for encryption?", "a": "Yes. The PDF and your chosen password are held in temp memory inside an isolated Docker container for the encryption pass only. Both the input plaintext and the password are unlinked the moment the encrypted file is delivered. Nothing is logged. The encryption pipeline is open source on GitHub."},
    ...
    "unlock-pdf": [
        ...
        {"q": "Is it safe to upload a sensitive locked PDF?", "a": "Yes. The encrypted PDF and your password are held in temp memory inside an isolated Docker container for the decryption pass only. Both are unlinked the moment your download begins. Nothing is logged, indexed, or sent to a third-party API. The decryption pipeline is open source on GitHub for verification."},
    ...
    "rotate-pdf": [
        ...
        {"q": "Is it safe to rotate a confidential PDF?", "a": "Yes. The PDF is held in temp memory inside an isolated Docker container for the rotation pass only — both input and output are unlinked the moment your download begins. Nothing is logged or persisted. The rotation pipeline is open source on GitHub."},
    ...
    "watermark": [
        ...
        {"q": "Is it safe to watermark a sensitive PDF?", "a": "Yes. The PDF and watermark image are held in temp memory inside an isolated Docker container for the watermarking pass only. Both are unlinked the moment your download begins. The watermark text or image is not stored, logged, or sent to a third party."},
    ...
    "redact-pdf": [
        ...
        {"q": "Is it safe to redact a sensitive legal document on PrivaTools?", "a": "Yes. The original and the redacted output are held in temp memory only and unlinked immediately after the response. The redaction logic is open source under MIT — you can read every line on GitHub or self-host the tool entirely. No third-party API is involved, no log captures content."},
    ],
    "flatten-pdf": [
    ...
    "pdf-to-word": [
        ...
        {"q": "Is it safe to convert a confidential PDF here?", "a": "Yes. The PDF and the generated .docx are held in a temp directory inside an isolated container for the duration of the request only — both are unlinked the moment the response is delivered. No copy is kept on any disk, in any log, or in any backup. The conversion code is open source under MIT for end-to-end audit."},
